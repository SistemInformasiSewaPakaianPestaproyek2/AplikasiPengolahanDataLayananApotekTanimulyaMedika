...
class Datadokteranak_model extends CI_Model {
	...
	{
		/*return $this->db->group_by('jabatan')
                        ->limit(1)
                        ->get($this->db_tabel)
                        ->row();*/
		
		
		return $this->db->select('data_dokter.kode_dokter,data_dokter.nama_dokter,data_dokter.ttl,data_dokter.alamat,data_dokter.str,data_dokter.telp,data_dokter.jabatan,data_dokter.jadwal,data_dokter.status')
					->from ($this->db_tabel)
	...
	public function buat_tabel($data)
	{
		...
		$row->jadwal,
		$row->status,
		anchor ('datadokteranak/edit/'.$row->kode_dokter,'Edit',array ('class'=>'edit'))
	...
	public function load_form_rules_edit()
    {
        $form_rules = array(
			...
			array(
               'field' => 'jabatan',
               'label' => 'Jabatan',
               'rules' => "required|callback_is_jabatan_exist"
            ),
			array(
               'field' => 'jadwal',
               'label' => 'Jadwal',
