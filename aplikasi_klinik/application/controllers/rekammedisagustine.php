...
class Rekammedisagustine extends CI_Controller
{
    public $data = array(
                        'modul'         => 'rekammedis',
                        ...
                        'pagination'    => '',
                        'tabel_data'    => '',
                        'main_view'     => 'rekammedis/rekammedis7',
                        'form_action'   => '',
                        'form_value'    => '',
                        'option_datapasien'  => '',
						'option_datadokter'  => '',
    ...
    public function index($offset = 0)
	{
        ...
        // cari data siswa
        $rekammedis = $this->rekammedis->cari_agustine($offset);

        // ada data siswa, tampilkan
        if ($rekammedis)
        {
            $tabel = $this->rekammedis->buat_tabel6($rekammedis);
        ...
        $this->load->view('template_agustine', $this->data);
	}
    ...
    public function tambah()
    {
        ...
        $this->data['form_action'] = 'rekammedisagustine/tambah';
        ...
        if($kode_pasien)
        {
            foreach($kode_pasien as $row)
            {
				...
				if($kode_pasien){
					$this->data[$row->kode_pasien] = $row->nama;
					} 
        }
        ...
        else
        {
		 ...
		 $nama_pasien = $this->datapasien->cari_semua();
        ...
        if($nama_pasien)
        {
            ...
            {
                $this->data['option_datapasien'][$row->nama] = $row->nama = $row->nama;
            }
        }
        // data kelas tidak ada
        else
        ...
        if($kode_dokter)
        {
            ...
            {
                $this->data['option_datadokter'][$row->kode_dokter ='140007'] = $row->nama_dokter = 'dr.Agustine';
            }
        }
        // data kelas tidak ada
        else
        ...
        if($this->input->post('submit'))
        {
            ...
            {
                if($this->rekammedis->tambahtommy())
                {
    ...
    public function edit($kode_rekam = NULL)
    {
        ...
        $this->data['main_view']   = 'rekammedis/rekammedis_form';
        $this->data['form_action'] = 'rekammedisagustine/edit/' . $kode_rekam;
	...
	function is_anamnesa_exist()
    {
        ...
        else
        {
            ...
            $query = $this->db->get_where('rekam_medis', array('anamnesa' => $anamnesa));
            if($query->num_rows() > 0)
            {
                $this->form_validation->set_message('is_anamnesa_exist',
                                                    "Anamnesa dengan $anamnesa_baru sudah terdaftar");
                return TRUE;
            }
            else
            {
	...
	function is_terapi_exist()
    {
        ...
        else
        {
            ...
            $query = $this->db->get_where('rekam_medis', array('terapi' => $terapi_baru));
            if($query->num_rows() > 0)
            {
                $this->form_validation->set_message('is_terapi_exist',
                                                    "Terapi dengan $terapi_baru sudah terdaftar");
                return TRUE;
            }
            else
            {
	...
	function is_pasien_exist()
    {
        ...
        else
        {
            ...
            $query = $this->db->get_where('rekam_medis', array('pasien' => $jadwal_baru));
            if($query->num_rows() > 0)
            {
                $this->form_validation->set_message('is_pasien_exist',
                                                    "Pasien dengan $pasien_baru sudah terdaftar");
                return TRUE;
            }
            else
            {
	...
	function is_kode_pasien_exist()
    {
        ...
        else
        {
            ...
            $query = $this->db->get_where('rekam_medis', array('kode_pasien' => $jadwal_baru));
            if($query->num_rows() > 0)
            {
	...
	function is_kode_dokter_exist()
    {
        ...
        else
        {
            ...
            $query = $this->db->get_where('rekam_medis', array('kode_dokter' => $jadwal_baru));
            if($query->num_rows() > 0)
            {
                $this->form_validation->set_message('is_kode_dokter_exist',
                                                    "Kode Dokter dengan $kode_dokter_baru sudah terdaftar");
                return TRUE;
            }
            else
            {
	...
	function is_alergi_exist()
    {
        ...
        else
        {
            ...
            $query = $this->db->get_where('rekam_medis', array('alergi' => $jadwal_baru));
            if($query->num_rows() > 0)
            {
                $this->form_validation->set_message('is_alergi_exist',
                                                    "Alergi dengan $alergi_baru sudah terdaftar");
                return TRUE;
            }
            else
            {
