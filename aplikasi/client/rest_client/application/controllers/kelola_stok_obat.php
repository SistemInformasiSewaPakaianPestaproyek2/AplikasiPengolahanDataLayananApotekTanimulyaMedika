<?php
Class Kelola_stok_obat extends CI_Controller{
   ...
   // var $API ="";
    ...
    function index(){
        $data=array ('title'=>'Klinik Tanimulya Medika',
         ...
         $data['data_stok'] = json_decode($this->curl->simple_get($this->API.'/data_stok'));
    ...
    function edit(){
        ...
        if(isset($_POST['submit'])){
             $data = array(
                ...
                'harga_jual_akhir'     =>  $this->input->post('harga_jual_akhir'),
                'nilai_laba_akhir'    =>  $this->input->post('nilai_laba_akhir')
                );
            $update =  $this->curl->simple_put($this->API.'/data_stok', $data, array(CURLOPT_BUFFERSIZE => 10)); 
            if($update)
            {
         ...
         // $data['nama_obat'] = json_decode($this->curl->simple_get($this->API.'/nama_obat'));
            ...
            $data['data_stok'] = json_decode($this->curl->simple_get($this->API.'/data_stok',$params));
            $this->load->view('layout/wrapper',$data);
        }
    }
